...
assert(/export const APP_RELEASE_VERSION = '2X\.1'/.test(publicSource));
...
assert.equal(witness.presentation.schemaVersion, '2X.1');
...
for (const required of ['npm run build', 'npm run lint', 'git diff --check', 'npm run diagnostic:phase2x:committed']) {
